  ...
  try {
    ...
    const result = await run(
      `INSERT INTO residents (resident_id, household_id, name, last_name, first_name, middle_name, birthdate, age_bracket, relation_to_head, sex, contact_number, is_pwd, is_pregnant_lactating)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [resident_id, household_id, name, last_name, first_name, middle_name || null, birthdate, age_bracket, relation_to_head || null, sex || null, contact_number || null, is_pwd ? 1 : 0, is_pregnant_lactating ? 1 : 0]
    )
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await run(
      ...
      [name, last_name, first_name, middle_name || null, birthdate, age_bracket, relation_to_head, sex || null, contact_number || null, is_pwd ? 1 : 0, is_pregnant_lactating ? 1 : 0, req.params.id]
    )
