...
export default function RequestAccount() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-900 via-primary-800 to-primary-700 flex items-center justify-center p-4 py-6">
      <div className="w-full max-w-lg">
        <div className="text-center mb-4">
          <div className="inline-flex items-center justify-center w-11 h-11 rounded-xl bg-white shadow-lg mb-2">
            <Shield size={20} className="text-primary-700" />
          </div>
          <h1 className="text-lg font-bold text-white leading-tight">PDRA</h1>
          <p className="text-blue-200 text-xs">Gingoog City CDRRMO</p>
        </div>

        <div className="bg-white rounded-2xl shadow-2xl p-5 sm:p-7">
          <Link to="/login" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-3">
            <ArrowLeft size={13} /> Back to Login
          </Link>

          {!done ? (
            <>
              <h2 className="text-lg font-semibold mb-1">Request an Account</h2>
              ...
              <form onSubmit={submit} className="space-y-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea className="input" rows={2} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} placeholder="Anything CDRRMO should know…" />
                  </div>
                </div>
